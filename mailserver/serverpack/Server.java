...
public class Server extends JFrame implements Runnable {
	...
	private void loadAccounts() {
		...
		try {
			...
			if(file.exists() == false) {
				area.append("Account file not found, there is no registered users.\n");
			}
			
			// If there is something (size not 0)
			else if(file.length() != 0) {
				BufferedReader accountReader = new BufferedReader(new FileReader("accounts.txt"));
			...
			// Otherwise
				...
				area.append("Accountfile was empty.\n");
			}
		}
		// When end of file is reached, nullpointerexception is thrown
			...
			area.append(accountMap.size() + " accounts loaded\n");
		}
	...
	private void saveAccounts() {
		try {
			...
			if(accountMap.isEmpty()) {
				area.append("No accounts to save.\n");
			}
			else {
				PrintWriter accountWriter;
				// update the registeredUsers set
				accountWriter = new PrintWriter(new File("accounts.txt"));
				...
				accountWriter.close();
				area.append(counter + " accounts saved\n");
			}
		}
	...
	private void showAccounts() {
		...
		if(registeredUsers.isEmpty()) {
			area.append("No registered users.\n");
		}
		else {
			area.append("List of registered users:\n");
	...
	private void startServer() {
		try {
			...
			serverThread = new Thread(Server.this);
			serverThread.start();
		...
		public void run() {
			...
			while(true) {
				...
				try {
					...
					// Login/Register
					if(receivedObject.getClass().equals("Class Login")) {
						...
						if(login.isRegistered() == true) {
							...
							if((activeAccount = accountMap.get(login.getName())) == null) {
								toClient.writeObject(new Error(1));
								closeAndClean();
							}
							// User already logged in
							else if((accountMap.get(login.getName()).isLoggedin()) == true) {
								toClient.writeObject(new Error(2));
								closeAndClean();
							}
							// Account found and it wasn't logged in
							else {
								// If password is incorrect
								if(activeAccount.getPass() != login.getPass()) {
									toClient.writeObject(new Error(3));
									closeAndClean();
								}
						...
						// Register as new user, account creation
							...
							if((accountMap.containsKey(login.getName())) == true) {
								toClient.writeObject(new Error(4));
								closeAndClean();
							}
							...
							// and update the registeredUsers-set
								...
								if(activeAccount.hasFolder() == false) {
									toClient.writeObject(new Error(5));
					...
					// Quitting 
					else if(receivedObject.getClass().equals("Class Error")) {
					...
					// Fetch something
					else if(receivedObject.getClass().equals("Class Get")) {
