...
*/		
int write_blocks_to_file(filedata *first,char *filename)
{
	int filed;
	int bytes_written = 0;
	filedata *prev;
	...
	if((filed = open(filename,O_CREAT|O_WRONLY,S_IRWXU)) == -1) error("Cannot create file or get writing rights to file");
...
filedata *read_file_to_blocks(char *filename)
{
	...
	if((filed = open(filename,O_RDONLY)) == -1) error("Cannot open file for reading");
